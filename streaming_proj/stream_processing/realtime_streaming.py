...
# Loading Kafka Cluster/Server details from configuration file(streaming.conf)
conf_file_path = "<path>"
conf_file_name = conf_file_path + "streaming.conf"
...
cassandra_host_name = config_obj.get('cassandra', 'host')
cassandra_port_no = config_obj.get('cassandra', 'port_no')
cassandra_keyspace_name = config_obj.get('cassandra', 'keyspace_name')
cassandra_table_name = config_obj.get('cassandra', 'table_name')
...
def save_to_cassandra(current_df, epoc_id):
    print("Inside save_to_cassandra function")
    print("Printing epoc_id: ")
    print(epoc_id)
    ...
    # Save the cassandra to the table.
    current_df.write \
    ...
    print("Exit out of save_to_cassandra function")
...
if __name__ == "__main__":
    ...
    # Construct a streaming DataFrame that reads from order-events
    orders_df = spark \
        .readStream \
        .format("kafka") \
    ...
    print("Printing schema of orders_df3 before creating date & hour column from order_datetime and storing to CAassandra")
    orders_df3.printSchema()

    orders_df3_write_stream =orders_df3 \
    .writeStream \
